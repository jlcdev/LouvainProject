...
public class Louvain extends Algorithm
{
    ...
    public void calc(Graph<Integer, Double> g)
    {
        ...
        while(control)
        {
            this.communityList.put(steps, community);
            steps++;
            control = this.metode();
        }
    }
    ...
    public ArrayList<ArrayList<Integer>> obtain()
    {
        ...
        else return this.communityList.get(this.p * this.steps / 100);
    }
    ...
    private void incluirNode(int node, int comunidad) 
    {
        ...
        {
            if(this.community.get(i).get(0) == node) nodeList = this.community.get(i);
            else if (this.community.get(i).get(0) == comunidad) posicio = i;
        }
        this.community.get(posicio).addAll(nodeList);
        
        
        ArrayList<Integer> neighbors = this.graph.getNeighbors(node);
        for (int i = 0; i < neighbors.size(); ++i) {
            int vecino = neighbors.get(i);
            Edge<Integer, Double> aVecinoNode = this.graph.getEdge(vecino, node);
